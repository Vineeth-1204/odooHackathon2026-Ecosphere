    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 relative overflow-hidden bg-white">
      ...
      <div className="relative w-full max-w-md glass-panel shadow-glass rounded-2xl p-8 z-10">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-[#24333E] tracking-tight">Forgot Password</h2>
          <p className="text-[#90998C] text-sm mt-1.5">Enter your email to receive a password reset link</p>
        </div>
        ...
        {success ? (
          <div className="flex flex-col gap-6 text-center animate-in zoom-in-95 duration-300">
            <div className="p-4 rounded-xl bg-brand-500/10 border border-brand-500/20 text-[#1F4032] flex flex-col items-center gap-3">
              <CheckCircle2 size={36} />
              <p className="text-sm font-medium leading-relaxed">{success}</p>
            </div>
            
            {resetToken && (
              <div className="p-4 rounded-lg bg-white border border-[#E4E6DF] text-left">
                <span className="text-[10px] font-bold text-[#1F4032] uppercase tracking-widest block mb-1">Developer Debug Token:</span>
                <code className="text-xs break-all text-[#24333E]">{resetToken}</code>
              </div>
            )}
